...
namespace PrestaShop\Module\HB_Hscode\CQRS\CommandBuilder;
 ...
 */
final class ProductHSCodeCommandsBuilder implements ProductCommandsBuilderInterface
{
    public function buildCommands(ProductId $productId, array $formData, ShopConstraint $singleShopConstraint): array
    {

        $command = null;
        if (isset($formData['details']['references']['hs_code'])) {
            $command = $this->getCommand($command, $productId->getValue())->setHsCode($formData['details']['references']['hs_code']);
        }

        return null !== $command ? [$command] : [];
    }

    private function getCommand(?UpdateProductHSCodeCommand $existingCommand, int $productId): UpdateProductHSCodeCommand
    {
        if (null !== $existingCommand) {
            return $existingCommand;
        }

        return new UpdateProductHSCodeCommand($productId);
    }
}
